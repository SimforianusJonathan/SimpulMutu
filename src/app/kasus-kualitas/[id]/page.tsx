...
export default async function QualityCasePage({
  ...
  const completeness = getStageCompleteness(
    ...
    qualityCase.contributingCauses.map(
        ...
        <div className="mt-10">
          ...
          {stage === "ringkasan" ? (
            <Summary
              ...
              linkedCauseCount={
                causes.filter((cause) => cause.evidenceIds.length > 0).length
              }
              problem={qualityCase.problem}
            />
          ) : null}

          {["akar", "tindakan"].includes(stage) ? (
            <LaterStage label={stages[stageKeys.indexOf(stage)]} />
          ) : null}
        </div>
      </section>
    </AppShell>
...
function LaterStage({ label }: { label: string }) {
  return (
    <section>
      <h2 className="text-2xl font-semibold">{label}</h2>
      <p className="mt-3 max-w-2xl leading-7 text-slate-600 dark:text-slate-300">
        Tahap ini adalah bagian dari alur investigasi, tetapi belum tersedia
        pada milestone ini. Tidak ada kesimpulan atau tindakan yang dibuat oleh
        sistem.
      </p>
    </section>
  );
...
function Summary({
  ...
  return (
    <section>
      <h2 className="text-2xl font-semibold">Ringkasan yang tersedia</h2>
      <dl className="mt-5 space-y-4 border-t border-slate-200 pt-5 dark:border-slate-800">
        <div>
        ...
        <div>
          ...
          <dd className="mt-1">
            {evidenceCount === 0
              ...
              : `${evidenceCount} Bukti telah dicatat.`}
          </dd>
        </div>
        <div>
          <dt className="text-sm font-semibold text-slate-600 dark:text-slate-300">
          ...
          <dd className="mt-1">
            {causeCount === 0
              ...
              : `${causeCount} Faktor Penyebab dicatat; ${linkedCauseCount} telah memiliki hubungan Bukti.`}
          </dd>
        </div>
      </dl>
      <p className="mt-6 text-sm leading-6 text-slate-600 dark:text-slate-300">
        Ringkasan ini hanya memuat informasi yang tersedia dan belum
        menunjukkan kesiapan untuk menyelesaikan Kasus Kualitas.
      </p>
    </section>
  );
}
